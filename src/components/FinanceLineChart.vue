<template>
  <div class="card h-full p-4 sm:p-5">
    <div class="mb-4">
      <h3 class="text-lg font-semibold text-primary">This Month Expenses</h3>
      <p class="text-sm text-secondary">
        Daily expense trend for {{ financeStore.selectedMonthLabel }}
      </p>
    </div>
...
const dailyTotals = computed(() => {
  ...
  return days.map(day => {
    ...
    const total = expenseEntries.value.reduce((sum, entry) => {
      return entry.date === key ? sum + Number(entry.value) : sum
    }, 0)
