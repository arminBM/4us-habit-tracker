<template>
  <div class="card h-full p-4 sm:p-5">
    <div class="mb-4">
      <h3 class="text-lg font-semibold text-primary">Sleep Duration</h3>
      <p class="text-sm text-secondary">
        Daily sleep (hours) for {{ financeStore.selectedMonthLabel }}
      </p>
    </div>
...
const chartWidth = computed(() => {
  if (props.exportMode) return 1400 // full export width
...
const dailyTotals = computed(() => {
  ...
  return days.map(day => {
    ...
    return {
      ...
      total: Number((totalMinutes / 60).toFixed(1)),
    }
  })
})
